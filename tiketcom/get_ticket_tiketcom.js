...
const PACKAGE_CODES   = ["2"];     // semua kode paket target dari API (packageCodes[] di tier, atau packages[].code)
const PACKAGE_HASH    = "2";       // index hash di URL: buka /packages → klik paket → lihat #pricetierDetail-{n}
...
// Pakai device_id yang sama dengan yang ada di SESSION_COOKIE
...
// Cek apakah package target ACTIVE dan product BOOKABLE
async function checkAvailability() {
    ...
    const productStatus = product.availabilityStatus;   // "BOOKABLE" | "SOLD_OUT" | "UPCOMING" | dll
    ...
    // ACTIVE jika salah satu kode sudah aktif
    const activePkg = matchedPkgs.find(p => p.productPackageStatus === "ACTIVE");
    const pkg       = activePkg || matchedPkgs[0];
    const pkgStatus = pkg.productPackageStatus;   // "ACTIVE" | "INACTIVE" | "SOLD_OUT" | dll
...
async function bookOnce(puppeteer, packageUrl, cookies) {
    const debugPort = 9222;
    const os   = require("os");
    const path = require("path");

    // Dedicated dir — bisa dibuka kapan saja tanpa konflik dengan Chrome yang sedang buka.
    // Session cookie diinjek otomatis sehingga user tetap logged in di tiket.com.
    const userDataDir = path.join(os.homedir(), "AppData", "Local", "Google", "Chrome", "ticket-sniper-book");

    // Launch Chrome sebagai proses independen (bukan child Node.js) agar tidak ikut mati saat Node exit
    ...
    // Set viewport besar agar halaman tidak render dalam mode mobile/narrow
    ...
    // Inject session cookies via CDP
    ...
    // Tambah quantity kalau lebih dari 1
    for (let i = 1; i < TARGET_QTY; i++) {
        const ok = await page.evaluate(() => {
            function click(el) {
                el.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true }));
            }

            // Strategy 1: aria-label (case-insensitive)
            ...
            // Strategy 2: button atau SVG dengan title/desc berisi plus/add
            ...
            // Naik hingga 6 level dari elemen "Pax"
    ...
    await browser.disconnect();  // putus koneksi, Chrome tetap hidup (proses independen)
}

async function openAndBook(packagesUrl) {
    const puppeteer = require("puppeteer-core");

    const packageUrl = `${packagesUrl}#pricetierDetail-${PACKAGE_HASH}`;
    const cookies    = SESSION_COOKIE.split(";")
        .map(raw => {
    ...
    await bookOnce(puppeteer, packageUrl, cookies);
}
...
(async () => {
    ...
    while (true) {
        ...
        // Paket aktif dan product bisa dibooking → buka browser
